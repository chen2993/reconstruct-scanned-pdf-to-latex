...
**所有可见编号都是 `.cls` 的职责，页面源码只提供内容。** 页面里出现 `(1)`、`（1）`、`A.`、`①`、`例 1.2` 这类编号字符即为违规：它们绕过计数器后无法跨页续号、无法交叉引用，也无法被做题本按题目筛选，而且编号样式一旦与原书不符就只能逐页返工。

| 编号 | 由谁生成 | 页面源码写什么 |
|---|---|---|
| 例题、习题、定义、定理、图、表、公式 | 带计数器的语义环境 | 只写内容，编号由环境显示 |
| 小问、分步、罗列、圈码 | 语义列表环境（标签由 `.cls` 的 `label=` 决定） | `\item` 后只写内容 |
| 选择题选项 | 选项命令（如 `\bookchoicesfour`） | 只传各选项内容，不写 `A`/`B` |

`.cls` 必须为此提供可直接调用的接口：小问/分步用语义列表环境，标签形状（`(\arabic*)`、`（\arabic*）`、罗马数字等）按代表页量取；选择题用选项命令，标签由计数器生成，**每次进入选项组重置选项计数器**，并按原书版式提供单行四列、二二网格、竖排等变体；圈码用列表或命令，不手打 `①`。完整写法与要点见 [page-authoring.md](page-authoring.md) 第 1.5 节，本文件不重复页面侧示例。

编号接口必须**编译验证**，不能只看源码：用同一道题写最小样例，分别渲染小问、各选项版式与圈码，确认标签形状、对齐、跨页续号都与代表页一致；每个公共接口按第 5 节的正交配置要求单独编译。随后由 `scripts/audit_hardcoded_numbers.py` 审计逐页源码，成组字面编号必须为零。
