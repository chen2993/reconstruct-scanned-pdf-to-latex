...
- 答案可以包含步骤、评注和答案专属媒体；
- 知识块可以包含证明、评注和媒体；
- 题目不能嵌入另一个题目，答案不能成为另一个答案的任意容器；跨页题目用显式 `start`、`continuation`、`end` 接口，不让一个环境跨越多个文件。
...
跨页知识块和题目必须记录开始、续页、结束状态；只有开始状态推进题号，只有结束状态在做题本中生成一次答题区。续页不得重复编号、题注或答题区。
...
默认每个并行单元负责连续 10 个正文页面。任务必须附当前样式摘要版本、可用卡片、源页标识和停工反馈格式。执行单元内部完成转写、内容复核、编译复核三轮，只修改分配文件，不修改 `.cls`、入口、集中表或 Git。
...
做题本的答题区由 `.cls` 集中测量和生成：完整题干、必要媒体和答题区作为一个分页单元，原书尺寸与 A4 自然向下但不得拆开，Pad11/Pad13 默认一页一题。答题区高度可以参考被隐藏答案的排版高度动态计算；若单页容量不足，优先整题移到下一页，超出单页容量时才按项目契约回退。测量或丢弃答案时必须保留局部分组、计数器、引用和完整 LaTeX 环境栈；语义所有者栈与普通环境栈分别审计，不能为了隐藏答案而删除未登记的 `tikzpicture`、公式或列表。
