...
前置和后置内容按逻辑模块逐条 `\input`，一个模块可以自然扩展为多页；不要为了对齐源扫描页而把目录、序言或参考文献硬拆成固定页数。原书纸型和版心由多张代表页共同推断，尺寸漂移或异常 `MediaBox` 只记录为待确认项，不直接写死进 `.cls`。
...
项目 `.cls` 的 `\bookinput` 接口负责按三位编号依次加载 `pages/pages-001.tex` 至 `pages/pages-584.tex`。若类文件实际使用不同命令，必须在 `docs/class-api.md` 中记录等价接口；前置和后置仍不得改成批量范围加载器。一个前置或后置模块可以在同一文件中自然生成多页。
...
- 答案可以包含步骤、评注和答案专属媒体；
- 知识块可以包含证明、评注和媒体；
- 题目不能嵌入另一个题目，答案不能成为另一个答案的任意容器；跨页对象保留同一个语义环境，可以跨越连续 `\input` 的多个源码文件，不能拆成公开的分段接口。
...
跨页知识块和题目使用单一、连续的 LaTeX 环境：在一个源码文件写 `\begin{...}`，在后续源码文件写匹配的 `\end{...}`。连续 `\input` 形成一个 TeX token stream；文件边界不是分组，也不自动代表成品分页。
...
- 这类环境必须是流式、非捕获正文的实现，例如经验证的 `\newenvironment`。`\NewEnviron` 和其他读取完整正文的装饰器不能用于它们，除非有专门的跨 `\input` 编译用例证明安全；
