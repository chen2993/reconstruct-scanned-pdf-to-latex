...
`front/toc.tex` 只能调用一次 `\\bookmaketoc` 自动目录指令，并由 `scripts/audit_toc.py` 审计；最终 PDF 必须有封面、前言、献词和书末页顶层书签，不能手工写目录页码或书签物理页。前后置模块的书签接口必须把目标放在模块实际第一页（参考接口会先开始新页）。
...
页目标契约。
最终 PDF outline 至少包含封面、前言、献词和书末页四个顶层书签；至少两遍编译后
...
- PDF outline（封面/前言/献词/书末页）复核状态：待记录
