...
页面重编号脚本会生成正文逐页 `.tex`、前后置逻辑模块骨架和 `latex/main.tex` 导入入口，但不会生成项目 `.cls`。
逐页源码和复核表使用 `front-xxx`、`pages-xxx`、`back-xxx` 页面标识及成品构建目标定位，不记录 PDF 物理页或逐页页码。
初始化后先记录页数、方向、`MediaBox` 范围、资源类型、扫描边框和页面分型；输入审计只读元数据、对象树和渲染图像，不读取文字层。
...
阶段 4 生成的唯一 `main.tex` 在 `\\documentclass` 前接收 driver 的 `\\BookBuildOptions`，再对前置页和后置页逐条写入原生 `\\input`，正文使用项目类文件的
`\\bookinput`（或文档化的等价命令）导入 `pages/pages-xxx.tex`。前置页和后置页使用
`cover`、`dedication`、`toc`、`afterword` 等经人工确认的逻辑模块文件名；一个模块可以自然扩展多页。
正文页使用 `pages/pages-xxx.tex`。在此记录页面文件名、导入顺序、缺页行为和可人工调整的顺序契约；
项目 `.cls` 不负责批量扫描或猜测页面文件。
...
把项目实际使用的所有者环境、`owner_parent_environments` 直接父级白名单、结构命令、
内部布局环境、题目环境、答案环境和答案专属媒体写入配置；集合中的值是完整替换而非
增量追加。白名单的键是子所有者，值是允许的直接父所有者数组，`$root` 表示根所有者。
...
对每种可跨页知识块和题目记录开始、续页、结束的合法状态转换、状态所有者、
环境边界、计数器步进和错误行为。一个逻辑题只能编号一次；做题本答题区只能由
约定的结束状态生成一次，续页不得重复编号或重复留白。

| 对象类型 | 开始接口 | 续页接口 | 结束接口 | 状态所有者与合法转换 | 编号/答题区契约 | 失败行为 | 验证用例 |
|---|---|---|---|---|---|---|---|
...
未选题目不得产生答题区，跨页逻辑题也只能在结束状态产生一次答题区。
...
SEMANTIC_AUDIT = {"owner_parent_environments": {}}
