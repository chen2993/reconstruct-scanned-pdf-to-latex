...
自定义环境、命令、计数器、标签键和配置 API 必须使用英文 ASCII 标识符；标准 LaTeX 的带星号布局环境只能按既有环境使用，不能给自定义语义所有者加星号；中文只作为内容或显示文本的值。
每个构建目标通过 driver 定义选项后再输入入口；入口可以是 `latex/main.tex`（单入口 + 类文件开关），也可以是做题本专用的独立入口（如 `main-workbook.tex` 加专用封面模块）——两种都支持，由用户按做题本与完整书的差异大小决定。不变的是：题目与答案内容只有一份源码，类文件只有一个。
`front/toc.tex` 只能调用一次 `\\bookmaketoc` 自动目录指令，并由 `scripts/audit_toc.py` 审计；原书实际存在的每个前后置模块都必须有对应顶层书签（有献词就要有，没有就不该出现），不能手工写目录页码或书签物理页。前后置模块的书签接口必须把目标放在模块实际第一页（参考接口会先开始新页）。
...
- 文件：待确定
- 基类与编译引擎：待确定
- 书内印刷规格证据（原文/开本/成品尺寸/单位/证据页处理结论/最终页面标识如保留）：待记录
- 经页面几何验证的原书纸张尺寸：待确认
...
`latex/front/toc.tex` 只调用一次项目类文件提供的自动目录指令（例如
`\\bookmaketoc`），不得手写目录条目、页码或逐页 `\\addcontentsline`。结构命令
必须自动写入目录和 PDF 书签；前后置模块在实际第一页使用
`\\bookbookmarkmodule{显示标题}{ascii_key}` 或记录在此处的等价接口。参考接口会先结束当前页并建立锚点；项目类改写时也必须保持模块首
页目标契约。
最终 PDF outline 覆盖原书实际存在的每个前后置模块书签；至少两遍编译后
检查 `.toc`、书签层级和目标位置均已收敛。缺少对应原件模块时暂停并人工决定，不能伪造。

## 编号接口

所有可见编号都由 `.cls` 生成，逐页源码只提供内容。逐项登记本项目的实际接口；
写法与「源码不得出现编号字符」的判据见
`references/contract/page-authoring.md` 第 1.5 节。

| 成组内容 | 实际接口 | 标签形式 | 计数器与重置 | 版式变体 | 编译验证 |
|---|---|---|---|---|---|
| 小问 / 分步 | 待填写（语义列表环境） | 待填写（如 `(\arabic*)` 或 `（\arabic*）`） | 待填写 | 待填写 | 待验证 |
| 选择题选项 | 待填写（选项命令） | 待填写（如 `A.` / `(A)`） | 待填写 | 待填写（四列 / 二二 / 竖排 / 括号版） | 待验证 |
| 圈码序号 | 待填写（列表或命令） | 待填写 | 待填写 | 待填写 | 待验证 |

验收条件：代表页确认标签形状后写进 `.cls`；每组选项进入时必须重置选项计数器；
用 `scripts/audit_hardcoded_numbers.py` 审计逐页源码，成组字面编号必须为零。
...
- 源 PDF SHA-256：待记录
- 输入审计状态：待记录
- 书内印刷规格（原文/开本/成品尺寸/单位/证据页处理结论/最终页面标识如保留）：待记录；舍去证据页不记录物理页号或临时页映射
- 页面方向与 `MediaBox` 范围（含尺寸漂移/异常结论）：待记录
- 页面资源类型与扫描边框结论：待记录
- 页面分型清单（仅记录实际存在的类型）：待记录
- 舍去页规则（`cover-facsimile` / `publication-info`）：待记录
- 拆页策略（auto/embedded/render）：待记录
- 回退渲染 DPI：待记录
- 页面命名范围（front-xxx/pages-xxx/back-xxx）：待记录
- 项目类名：待记录
- 自动目录指令与 `.toc` 收敛状态：待记录
- PDF outline（按原书实际存在的前后置模块逐个核对）复核状态：待记录
